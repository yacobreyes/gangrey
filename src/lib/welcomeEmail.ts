...
export async function sendWelcomeEmail(to: string): Promise<void> {
  ...
  const html = submissionEmailHtml(
    `<p style="font-size:17px;line-height:1.7;color:#000000 !important;margin:0 0 20px;">
     ...
     <p style="font-size:17px;line-height:1.7;color:#000000 !important;margin:20px 0 0;">
       From here you'll get our monthly Gangrey classics newsletter, and our issues when they launch.
     </p>
  ...
  const { error } = await resend.emails.send({ from, to: [to], subject: "Welcome to Gangrey", html });
