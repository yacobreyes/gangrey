...
// Irregular torn top/bottom edges for the archive clippings — the exact
// polygons from the design reference (straight left/right, uneven notches).
const TORN = [
  ...
  const cover = opts.masthead
    ? `<div style="background:${GROUND};padding:16px;">
        <div style="border:1px solid ${RULE};padding:26px 34px 24px;">
          ...
          ${author ? `<p style="font-family:${FONT};font-size:10px;letter-spacing:0.22em;text-transform:uppercase;color:${RULE};text-align:center;margin:0 0 24px;">Guest Editor · <span style="color:#ffffff;">${esc(author)}</span></p>` : `<div style="height:8px;"></div>`}
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-top:1px solid ${RULE};"><tr>
            ...
            <td align="right" style="padding-top:16px;font-family:${SERIF};font-size:11px;letter-spacing:0.06em;color:#ffffff;">${volume ? `<span style="color:${RULE};">Vol.</span> ${esc(volume)}` : ""}${volume && issue ? " &nbsp; " : ""}${issue ? `<span style="color:${RULE};">No.</span> ${esc(issue)}` : ""}</td>
          </tr></table>
        </div>
      </div>`
    : "";
