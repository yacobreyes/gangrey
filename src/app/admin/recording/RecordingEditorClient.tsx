...
export default function RecordingEditorClient({ initialLines, initialClips, loadError }: {
  ...
  // draft end — so you can audition before saving.
  function playPreview(c: RecordingClip, d: { start: string; end: string }) {
    ...
    stopTimer.current = setInterval(() => {
      if (end !== null && audio.currentTime >= end) stopPreview();
    }, 40);
  }
  ...
  async function saveClip(index: number) {
    ...
    const start = num(d.start), end = num(d.end);
    if ([start, end].some(v => v !== null && !Number.isFinite(v))) {
    ...
    <div style={{ minHeight: "100vh", background: "#f4f1ea", fontFamily: FONT, color: "#111" }}>
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "2.2rem 1.2rem 5rem" }}>
        ...
        <p style={{ color: MUTED, fontSize: "0.8rem", margin: "0 0 0.8rem", lineHeight: 1.5 }}>
          Start/End are seconds into the audio file (decimals fine, e.g. 1.35). Blank = play the whole file.
          Press ▶ to hear it with your current numbers before saving.
        </p>
