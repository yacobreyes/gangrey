...
import { setPostStage, setPostAssignee } from "../actions";
...
export default function EditorialCalendar({ initialUsers = [] }: { initialUsers?: FlatplanUser[] }) {
  ...
  // Current calendar month (first-of-month, UTC-safe).
      ...
      {loading ? <p style={{ fontFamily: FONT, color: TEXT_MUTED }}>Loading…</p> : view === "board" ? (
        ...
        <div style={{ background: "white", border: `1px solid ${BORDER}`, borderRadius: 14, overflow: "hidden", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0.9rem 1.1rem", borderBottom: `1px solid ${BORDER}` }}>
            <button onClick={() => setCursor(c => new Date(c.getFullYear(), c.getMonth() - 1, 1))} style={{ background: "none", border: "none", cursor: "pointer", color: TEXT_MUTED, display: "flex", padding: 4 }}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round"><polyline points="15 18 9 12 15 6"/></svg></button>
            <span style={{ fontFamily: "var(--font-headline)", fontSize: "1.15rem", fontWeight: 800, color: TEXT_DARK }}>{MONTHS[cursor.getMonth()]} {cursor.getFullYear()}</span>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)" }}>
            ...
            {monthGrid.map((day, i) => (
              <div key={i} style={{ minHeight: 92, borderRight: (i % 7 !== 6) ? `1px solid #eee` : "none", borderBottom: `1px solid #eee`, padding: "0.35rem", background: day === todayStr ? "#fffdf7" : "white" }}>
                {day && <>
                  ...
                  {(postsByDay[day] ?? []).slice(0, 4).map(p => (
                    <div key={p._id} onClick={() => router.push(`/admin/imago/posts/${p.slug}`)} title={p.headline}
                      style={{ display: "flex", alignItems: "center", gap: 4, cursor: "pointer", marginBottom: 2 }}>
                      <span style={{ width: 6, height: 6, borderRadius: "50%", background: STATUS_DOT[p.status ?? "published"] ?? TEXT_MUTED, flexShrink: 0 }} />
                      <span style={{ fontFamily: FONT, fontSize: "0.7rem", color: TEXT_DARK, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.headline || "Untitled"}</span>
                    </div>
                  ))}
